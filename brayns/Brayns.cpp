...
struct Brayns::Impl {
  Impl(int argc, const char **argv) : _engine(nullptr) {
    ...
    _engineFactory.reset(new EngineFactory(argc, argv, *_parametersManager));
    createEngine();
  ...
  */
  void _loadXYZBFile() {
    ...
    if (!xyzbLoader.importFromBinaryFile(geometryParameters.getXYZBFile(),
                                         scene))
      BRAYNS_ERROR << "Failed to import " << geometryParameters.getXYZBFile()
                   << std::endl;
  }
  ...
  */
  void _loadMeshFolder(const std::string &folder) {
    ...
    for (const auto &file : files) {
      ...
      if (!_meshLoader.importMeshFromFile(file, scene, quality, Vector3f(),
                                          Vector3f(1, 1, 1), material))
        BRAYNS_ERROR << "Failed to import " << file << std::endl;
      ++progress;
    }
#else
    BRAYNS_ERROR << "Assimp library is required to load meshes from " << folder
                 << std::endl;
#endif
  }
  ...
  */
...
#else
    BRAYNS_ERROR << "Assimp library missing for molecular meshes" << std::endl;
#endif
  }
  ...
  void _setupCameraManipulator(const CameraMode mode) {
    ...
    case CameraMode::flying:
      _cameraManipulator.reset(
          new FlyingModeManipulator(_engine->getCamera(), *_keyboardHandler));
      break;
...
Brayns::Brayns(int argc, const char **argv) : _impl(new Impl(argc, argv)) {}
...
void Brayns::render() { _impl->render(); }

Engine &Brayns::getEngine() { return _impl->getEngine(); }
