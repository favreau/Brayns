...
Scene::Scene(Renderers renderers, ParametersManager &parametersManager)
    : _parametersManager(parametersManager), _renderers(renderers),
      _spheresDirty(true), _cylindersDirty(true), _conesDirty(true),
      _trianglesMeshesDirty(true), _volumeHandler(0), _simulationHandler(0) {}
  ...
  for (size_t i = 0; i < nbMaterials; ++i) {
    ...
    case MT_DEFAULT:
      ...
      case 4: // Apical dendrite
        ...
        break;
      case MATERIAL_AFFERENT_SYNAPSE:
        material->setColor(Vector3f(1.f, 0.f, 0.f));
        break;
      case MATERIAL_EFFERENT_SYNAPSE:
    ...
    case MT_GRADIENT:
      material->setColor(Vector3f(
          float(std::rand() % nbMaterials) / float(nbMaterials), 1.f, 0.f));
      break;
    case MT_PASTEL_COLORS:
      material->setColor(Vector3f(0.5f + float(std::rand() % 127) / 255.f,
...
MaterialPtr Scene::getMaterial(size_t index) { return _materials[index]; }
...
void Scene::buildDefault() {
  ...
  for (size_t material = 1; material < 6; ++material) {
    ...
    _trianglesMeshes[material].getIndices().push_back(Vector3ui(0, 1, 2));
    _trianglesMeshes[material].getIndices().push_back(Vector3ui(3, 4, 5));
  }
  ...
  _cylinders[material].push_back(
      CylinderPtr(new Cylinder(material, Vector3f(0.25f, 0.126f, 0.75f),
...
void Scene::buildEnvironment() {
  ...
  case SceneEnvironment::ground: {
    ...
    v = Vector4f(c.x() - s.x() * scale.x(), c.y() - s.y() * scale.y() * 1.001f,
                 c.z() - s.z() * scale.z(), 0.f);
...
void Scene::clearLights() { _lights.clear(); }
