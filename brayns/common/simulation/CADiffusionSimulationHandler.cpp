...
{
    BRAYNS_DEBUG << "Loading Calcium simulation from " << simulationFolder << std::endl;
...
bool CADiffusionSimulationHandler::_loadCalciumPositions(
    const size_t frame )
{
    ...
    BRAYNS_DEBUG << "Loading Calcium positions for frame " << frame
                 << ", filename: " << _simulationFiles[frame]
                 << std::endl;
    ...
    {
        BRAYNS_ERROR << "Could not open file "
    ...
    filePositions.close();
    BRAYNS_DEBUG << _calciumPositions.size()
                 << " Calcium positions loaded" << std::endl;
    return true;
}
    ...
    if( !_spheresCreated )
    {
        ...
        for( const auto position: _calciumPositions )
        {
            SpherePtr sphere( new Sphere(
                MATERIAL_CA_SIMULATION, position, CALCIUM_RADIUS, 0.f, 0.f));
