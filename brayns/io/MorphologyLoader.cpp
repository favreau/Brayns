...
bool MorphologyLoader::_importMorphology(
  ...
  try {
    ...
    BRAYNS_DEBUG << "Normalized position : "
                 ...
                 << std::endl;
    BRAYNS_DEBUG << "Distance to soma    : " << maxDistanceToSoma << std::endl;
...
bool MorphologyLoader::importCircuit(const servus::URI &circuitConfig,
  ...
  _outputFile.open(morphologyFilebname.str(), std::ios::out | std::ios::binary);
#endif
...
#pragma omp for nowait
    for (size_t i = 0; i < uris.size(); ++i) {
      ...
      if (_importMorphology(uri, i, transforms[i], 0, private_spheres,
                            private_cylinders, private_cones, private_bounds,
                            simulationOffset, maxDistanceToSoma, minRadius)) {
...
#ifdef EXPORT_TO_FILE
  std::ofstream outputFile("morphology.bin", std::ios::out | std::ios::binary);
#endif
