...
#ifdef BRAYNS_USE_BRION
brain::neuron::SectionTypes
_getSectionTypes(const size_t morphologySectionTypes) {
...
void _createSpines(const brain::Circuit &circuit, const brain::GIDSet &gids,
                   const float radius, SpheresMap &spheres,
    ...
    for (const auto &synapse : synapses) {
      if (synapse.getPostsynapticSectionID() != 0) {
        ...
        spheres[MATERIAL_AFFERENT_SYNAPSE].push_back(SpherePtr(new Sphere(
            MATERIAL_AFFERENT_SYNAPSE, from, 4.f * radius, 0.f, 0.f)));
        cylinders[MATERIAL_AFFERENT_SYNAPSE].push_back(CylinderPtr(new Cylinder(
            MATERIAL_AFFERENT_SYNAPSE, from, to, 2.f * radius, 0.f, 0.f)));
        bounds.merge(from);
        bounds.merge(to);
      }
      ++i;
    ...
    for (const auto &synapse : synapses) {
      if (synapse.getPostsynapticSectionID() != 0) {
        ...
        spheres[MATERIAL_EFFERENT_SYNAPSE].push_back(SpherePtr(new Sphere(
            MATERIAL_EFFERENT_SYNAPSE, from, 4.f * radius, 0.f, 0.f)));
        cylinders[MATERIAL_EFFERENT_SYNAPSE].push_back(CylinderPtr(new Cylinder(
            MATERIAL_EFFERENT_SYNAPSE, from, to, 1.5f * radius, 0.f, 0.f)));
        bounds.merge(from);
        bounds.merge(to);
      }
      ++i;
...
bool MorphologyLoader::_importMorphologyAsMesh(const servus::URI &source,
  ...
  try {
    ...
    for (size_t s = 0; s < sections.size(); ++s) {
      ...
      const size_t samplesToProcess = std::min(samplesFromSoma, samples.size());
    ...
    const size_t material = _getMaterialFromSectionType(
        morphologyIndex, size_t(brain::neuron::SectionType::soma));
    metaballsGenerator.generateMesh(metaballs, gridSize, threshold, materials,
                                    material, meshes);
...
bool MorphologyLoader::importMorphology(const servus::URI &uri,
  ...
  if (_geometryParameters.useMetaballs()) {
    returnValue = _importMorphologyAsMesh(
        uri, morphologyIndex, scene.getMaterials(), Matrix4f(),
        scene.getTriangleMeshes(), scene.getWorldBounds());
  }
...
bool MorphologyLoader::_importMorphology(
  ...
  try {
    ...
    const MorphologyLayout &layout = _geometryParameters.getMorphologyLayout();
      ...
      for (size_t i = step; !done && i < samples.size() + step; i += step) {
        ...
        if (radius > 0.f)
          spheres[material].push_back(SpherePtr(
              new Sphere(material, position, radius, distance, offset)));

        bounds.merge(position);
        if (position != target && radius > 0.f && previousRadius > 0.f) {
          if (radius == previousRadius)
            cylinders[material].push_back(CylinderPtr(new Cylinder(
                material, position, target, radius, distance, offset)));
          else
            cones[material].push_back(
                ConePtr(new Cone(material, position, target, radius,
...
bool MorphologyLoader::importCircuit(const servus::URI &circuitConfig,
  ...
  std::map<uint32_t, size_t> neuronTypes;
#if 1
  ...
  // const brion::NeuronMatrix& neuronMatrix = brionCircuit.get( gids,
  // brion::NEURON_LAYER );
  // Identify the number of cells per M-type
  const std::string neuronTypesFilename =
      "/gpfs/bbp.cscs.ch/home/favreau/medias/neuronTypes/testdata/mtypes.txt";
  std::ofstream neuronTypesFile(neuronTypesFilename, std::ios::out);
  if (neuronTypesFile.good()) {
    for (size_t i = 0; i < gids.size(); ++i) {
      neuronTypes[i] = boost::lexical_cast<size_t>(neuronMatrix[i][0]);
      neuronTypesFile << i << " " << neuronTypes[i] << std::endl;
    }
  ...
  const std::string neuronTypesFilename =
      "/gpfs/bbp.cscs.ch/home/favreau/medias/neuronTypes/testdata/mtypes.txt";
  std::ifstream neuronTypesFile(neuronTypesFilename, std::ios::in);
  if (!neuronTypesFile.good()) {
    BRAYNS_ERROR << "Could not open file" << std::endl;
    return false;
  }

  std::string line;
  while (neuronTypesFile.good() && std::getline(neuronTypesFile, line)) {
    std::stringstream lineStream(line);
    uint32_t gid;
    size_t neuronType;
    lineStream >> gid >> neuronType;

    neuronTypes[gid] = neuronType;
  }
  neuronTypesFile.close();
#endif
...
#pragma omp parallel
  {
    ...
    TrianglesMeshMap private_triangleMeshes;
    Boxf private_bounds;
#pragma omp for nowait
    for (size_t i = 0; i < uris.size(); ++i) {
      ...
      if (_geometryParameters.useMetaballs()) {
        _importMorphologyAsMesh(uri, neuronTypes[i], scene.getMaterials(),
                                transforms[i], private_triangleMeshes,
                                private_bounds);
      }

      if (_importMorphology(uri, neuronTypes[i], transforms[i], 0,
...
#pragma omp critical
    for (const auto &p : private_spheres) {
      ...
      scene.getSpheres()[material].insert(scene.getSpheres()[material].end(),
...
#pragma omp critical
    for (const auto &p : private_triangleMeshes) {
      const size_t material = p.first;
      scene.getTriangleMeshes()[material].getIndices().insert(
          scene.getTriangleMeshes()[material].getIndices().end(),
          private_triangleMeshes[material].getIndices().begin(),
          private_triangleMeshes[material].getIndices().end());
      scene.getTriangleMeshes()[material].getVertices().insert(
          scene.getTriangleMeshes()[material].getVertices().end(),
          private_triangleMeshes[material].getVertices().begin(),
          private_triangleMeshes[material].getVertices().end());
      scene.getTriangleMeshes()[material].getNormals().insert(
          scene.getTriangleMeshes()[material].getNormals().end(),
          private_triangleMeshes[material].getNormals().begin(),
          private_triangleMeshes[material].getNormals().end());
      scene.getTriangleMeshes()[material].getTextureCoordinates().insert(
          scene.getTriangleMeshes()[material].getTextureCoordinates().end(),
          private_triangleMeshes[material].getTextureCoordinates().begin(),
          private_triangleMeshes[material].getTextureCoordinates().end());
    }

    scene.getWorldBounds().merge(private_bounds);
  }
  _createSpines(circuit, gids, minRadius, scene.getSpheres(),
...
#pragma omp for nowait
    for (size_t i = 0; i < cr_uris.size(); ++i) {
      ...
      if (_geometryParameters.useMetaballs()) {
        _importMorphologyAsMesh(uri, i, scene.getMaterials(), transforms[i],
                                private_triangleMeshes, scene.getWorldBounds());
      }
...
#pragma omp for nowait
      for (size_t i = 0; i < nonSimulatedCells; ++i) {
        ...
        _importMorphology(uri, i, allTransforms[i], 0, private_spheres,
                          private_cylinders, private_cones, private_bounds, 0,
                          maxDistanceToSoma, minRadius);
...
bool MorphologyLoader::importMorphology(const servus::URI &, const int,
                                        Scene &) {
...
size_t
MorphologyLoader::_getMaterialFromSectionType(const size_t morphologyIndex,
  ...
  case ColorScheme::neuron_by_segment_type:
    material = morphologyIndex * 5 +
               sectionType % (NB_MAX_MATERIALS - NB_SYSTEM_MATERIALS);
    break;
  default:
    material = 0;
  }
