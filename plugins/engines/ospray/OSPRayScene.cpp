...
OSPRayScene::OSPRayScene(Renderers renderers,
    ...
    : Scene(renderers, parametersManager), _ospLightData(0),
      _ospMaterialData(0), _ospVolumeData(0), _ospSimulationData(0),
      _ospTransferFunctionDiffuseData(0), _ospTransferFunctionEmissionData(0) {}
...
void OSPRayScene::_saveCacheFile() {
  ...
  for (size_t materialId = 0; materialId < nbMaterials; ++materialId) {
    ...
    file.write((char *)&bufferSize, sizeof(size_t));
    file.write((char *)_serializedSpheresData[materialId].data(), bufferSize);
    if (bufferSize != 0)
    ...
    file.write((char *)&bufferSize, sizeof(size_t));
    file.write((char *)_serializedCylindersData[materialId].data(), bufferSize);
    if (bufferSize != 0)
      BRAYNS_DEBUG << "[" << materialId << "] "
                   << _serializedCylindersDataSize[materialId] << " Cylinders"
                   << std::endl;
    ...
    if (_trianglesMeshes.find(materialId) != _trianglesMeshes.end()) {
      // Vertices
      bufferSize =
          _trianglesMeshes[materialId].getVertices().size() * sizeof(Vector3f);
      file.write((char *)&bufferSize, sizeof(size_t));
      ...
      // Indices
      bufferSize =
          _trianglesMeshes[materialId].getIndices().size() * sizeof(Vector3ui);
      file.write((char *)&bufferSize, sizeof(size_t));
      ...
      // Normals
      bufferSize =
          _trianglesMeshes[materialId].getNormals().size() * sizeof(Vector3f);
      file.write((char *)&bufferSize, sizeof(size_t));
      ...
      // Texture coordinates
      bufferSize = _trianglesMeshes[materialId].getTextureCoordinates().size() *
                   sizeof(Vector2f);
      file.write((char *)&bufferSize, sizeof(size_t));
      file.write(
...
void OSPRayScene::_loadCacheFile() {
  ...
  for (size_t materialId = 0; materialId < nbMaterials; ++materialId) {
    ...
    if (bufferSize != 0) {
      ...
      file.read((char *)_serializedSpheresData[materialId].data(), bufferSize);
    }
    ...
    if (bufferSize != 0) {
      ...
      file.read((char *)_serializedConesData[materialId].data(), bufferSize);
    }
...
uint64_t OSPRayScene::_serializeSpheres(const size_t materialId) {
  ...
  if (_spheres.find(materialId) != _spheres.end())
    for (const auto &sphere : _spheres[materialId]) {
      const float ts = (_models.size() == 1) ? 0.f : sphere->getTimestamp();
  ...
  for (const auto &timestampSpheresIndex :
    ...
    for (const auto &model : _models) {
      if (timestampSpheresIndex.first <= model.first) {
        ...
        ospSet1i(_ospExtendedSpheres[materialId], "bytes_per_extended_sphere",
                 Sphere::getSerializationSize() * sizeof(float));
        ospSet1i(_ospExtendedSpheres[materialId], "materialID", materialId);
...
uint64_t OSPRayScene::_serializeCylinders(const size_t materialId) {
  ...
  if (_cylinders.find(materialId) != _cylinders.end())
    for (const auto &cylinder : _cylinders[materialId]) {
      const float ts = (_models.size() == 1) ? 0.f : cylinder->getTimestamp();
      size += cylinder->serializeData(_serializedCylindersData[materialId]);
      ++_serializedCylindersDataSize[materialId];
  ...
  for (const auto &timestampCylindersIndex :
    ...
    for (const auto &model : _models) {
      if (timestampCylindersIndex.first <= model.first) {
        ...
        ospSet1i(_ospExtendedCylinders[materialId], "materialID", materialId);
    ...
    for (const auto &model : _models) {
      if (timestampConesIndex.first <= model.first) {
        ...
        _ospExtendedConesData[materialId] = ospNewData(
            conesBufferSize, OSP_FLOAT, &_serializedConesData[materialId][0],
            OSP_DATA_SHARED_BUFFER);

        ospSet1i(_ospExtendedCones[materialId], "materialID", materialId);
        ospSetObject(_ospExtendedCones[materialId], "extendedcones",
                     _ospExtendedConesData[materialId]);
        ospSet1i(_ospExtendedCones[materialId], "bytes_per_extended_cone",
                 Cone::getSerializationSize() * sizeof(float));
...
uint64_t OSPRayScene::_buildMeshOSPGeometry(const size_t materialId) {
  ...
  if (_trianglesMeshes.find(materialId) != _trianglesMeshes.end()) {
    ...
    size +=
        _trianglesMeshes[materialId].getVertices().size() * 3 * sizeof(float);
    OSPData vertices = ospNewData(
        _trianglesMeshes[materialId].getVertices().size(), OSP_FLOAT3,
        &_trianglesMeshes[materialId].getVertices()[0], OSP_DATA_SHARED_BUFFER);

    size +=
        _trianglesMeshes[materialId].getNormals().size() * 3 * sizeof(float);
    OSPData normals = ospNewData(
        _trianglesMeshes[materialId].getNormals().size(), OSP_FLOAT3,
        &_trianglesMeshes[materialId].getNormals()[0], OSP_DATA_SHARED_BUFFER);

    size += _trianglesMeshes[materialId].getIndices().size() * 3 * sizeof(int);
    ...
    size += _trianglesMeshes[materialId].getColors().size() * 4 * sizeof(float);
    ...
    size += _trianglesMeshes[materialId].getTextureCoordinates().size() * 2 *
            sizeof(float);
...
void OSPRayScene::commitLights() {
  ...
  for (auto renderer : _renderers) {
    ...
    if (_ospLightData == 0) {
      _ospLightData = ospNewData(_lights.size(), OSP_OBJECT, &_ospLights[0],
                                 OSP_DATA_SHARED_BUFFER);
  ...
  for (const auto &renderer : _renderers) {
    ...
    if (!updateOnly) {
      if (_ospMaterialData == 0) {
        _ospMaterialData =
            ospNewData(NB_SYSTEM_MATERIALS, OSP_OBJECT,
                       &_ospMaterials[MATERIAL_SYSTEM], OSP_DATA_SHARED_BUFFER);
...
void OSPRayScene::commitVolumeData() {
  ...
  if (data) {
    for (const auto &renderer : _renderers) {
      ...
      ospSet3f(osprayRenderer->impl(), "volumeOffset", offset.x(), offset.y(),
               offset.z());
...
void OSPRayScene::saveSceneToCacheFile() { _saveCacheFile(); }
