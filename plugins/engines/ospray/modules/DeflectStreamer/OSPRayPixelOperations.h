...
class OSPRayPixelOperations : public ospray::PixelOp
{
public:
    OSPRayPixelOperations();
        ...
        virtual void postAccum(Tile &tile)
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    // first, 'clip' the tile and write 'clipped' one to the
                    // left side
                    ...
                    // now, move right true to the left, clip on lower side, and
                    // shift pixels
                    plainTile.region.lower.x = 0;
                    plainTile.region.upper.x =
                        original_upper_x - trueScreenWidth;
                    // since pixels didn't start at 'trueWidth' but at
                    // 'lower.x', we have to shift
                    int numPixelsInRegion =
                        plainTile.region.size().y * plainTile.pitch;
